...
Fireshare exposes a public RSS feed at `/feed` that lists all videos marked as public, ordered by date. This feed is suitable for podcast clients, feed readers, or any tool that supports RSS. The feed title and description are configured via the `rss_config` section in `config.json`.
